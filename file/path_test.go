...
func TestFileUtils(t *testing.T) {
	...
	t.Run("TestExists", func(t *testing.T) {
		...
		for _, test := range tests {
			...
			if test.expectedError {
				log.Error(realError.Error())
			} else {
				t.Log(test.expectedValue, realValued, "Failed to test with '%s': %s", test.fileName, test.name)
			}
		}
	})
	...
	t.Run("TestFileOrSymlinkExists", func(t *testing.T) {
		...
		for _, test := range tests {
			...
			if test.expectedError {
				t.Errorf("Failed to test with '%s': %s", test.fileName, test.name, realError)
			} else {
				t.Log(t, test.expectedValue, realValued, "Failed to test with '%s': %s", test.fileName, test.name)
			}
		}
	})
	...
	t.Run("TestReadDirNoStat", func(t *testing.T) {
		...
		for _, test := range tests {
			...
			if test.expectedError {
				t.Errorf(realError.Error(), "Failed to test with '%s': %s", test.dirName, test.name)
			} else {
				t.Log(t, test.expectedValue, realValued, "Failed to test with '%s': %s", test.dirName, test.name)
			}
		}
	})
}
